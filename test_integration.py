...
def test_imports():
    ...
    try:
        ...
        print(f"  ⚠ PyQt6 导入失败（将跳过UI相关测试）: {e}")
    ...
    try:
        ...
        print(f"  ⚠ PyPDF2 导入失败（将跳过依赖PDF读取的测试）: {e}")
    
    try:
        from core.rename_engine import RenameEngine
        print("  ✓ RenameEngine 导入成功")
    except Exception as e:
        print(f"  ✗ RenameEngine 导入失败: {e}")
        return False
    
    try:
        from core.pdf_split_engine import PdfSplitEngine, SplitMode
        print("  ✓ PdfSplitEngine 导入成功")
    except Exception as e:
        print(f"  ✗ PdfSplitEngine 导入失败: {e}")
        return False
    ...
    if HAVE_PYQT6:
        ...
        try:
            ...
            print("  ✓ RenamePanel 导入成功")
        except Exception as e:
            print(f"  ✗ RenamePanel 导入失败: {e}")
            return False
    
    if HAVE_PYQT6:
        try:
            from ui.pdf_split_panel import PdfSplitPanel
            print("  ✓ PdfSplitPanel 导入成功")
        except Exception as e:
            print(f"  ✗ PdfSplitPanel 导入失败: {e}")
            return False

        try:
            from ui.about_panel import AboutPanel
            print("  ✓ AboutPanel 导入成功")
        except Exception as e:
            print(f"  ✗ AboutPanel 导入失败: {e}")
            return False
...
def test_rename_engine():
    ...
    try:
        ...
        if result["total"] == 3 and result["successful"] == 3:
            print(f"  ✓ 重命名执行成功，处理 {result['successful']}/{result['total']} 个文件")
        else:
            print(f"  ✗ 重命名执行失败，成功 {result['successful']}/{result['total']}")
            if result["errors"]:
        ...
        try:
            ...
            if new_name.endswith(".txt") and len(new_name) > 4:
                ...
                print(f"  ✗ 删除/保留规则预览异常: {new_name}")
                return False
        except Exception as e:
            print(f"  ✗ 删除/保留规则测试失败: {e}")
            return False
...
def test_rename_all_rules():
    ...
    try:
        ...
        try:
            ...
            print("  ⚠ 跳过PDF元数据相关智能识别测试（PyPDF2不可用）")
        ...
        for name, rules, input_filename, file_index, expected, filepath in cases:
            ...
            if out != expected:
                print(f"  ✗ {name} 失败: {input_filename} -> {out} (预期 {expected})")
                return False

        print(f"  ✓ 覆盖 {len(cases)} 个规则用例通过")
        return True
    finally:
        shutil.rmtree(temp_dir, ignore_errors=True)
...
def test_pdf_split_engine():
    ...
    try:
        ...
        if len(ranges) == 2:
            print(f"  ✓ 页面范围解析成功，解析出 {len(ranges)} 个范围")
        else:
            print(f"  ✗ 页面范围解析失败，预期 2 个范围，实际 {len(ranges)} 个")
            return False
    except Exception as e:
        print(f"  ✗ 页面范围解析失败: {e}")
        return False
            ...
            if rename_panel is not None:
                ...
                if rename_panel.file_list.topLevelItemCount() > 0:
                    ...
                    if first.text(1):
                        ...
                        print("  ✗ 重命名实时预览未生成预览结果")
                        return False

                os.remove(p1)
                os.remove(p2)
        except Exception as e:
            print(f"  ✗ 重命名实时预览测试失败: {e}")
            return False
    ...
    try:
        ...
        try:
            ...
            if not (first_name.endswith("file2.txt") and second_name.endswith("file10.txt")):
                print(f"  ✗ 文件名自然排序异常: {first_name} , {second_name}")
                return False
        ...
        try:
            ...
            pdf_panel._on_preview()
            preview_text = pdf_panel.preview_text.toPlainText().strip()
            if not preview_text or "test.pdf" not in preview_text:
                print("  ✗ PDF拆分预览未生成或内容异常")
                return False
